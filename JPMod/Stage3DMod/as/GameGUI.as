...
class GameGUI extends TextField
{
	...
	public function GameGUI(title:String = "", inX:Number=8, inY:Number=8, inCol:int = 0xFFFFFF)
	{
		...
		width = 800;
		selectable = false;
		defaultTextFormat = new TextFormat("_sans", 18, 0, true);
		text = "";
		textColor = inCol;
	...
	private function onEnterFrame(evt:Event):void
	{
		...
		if( timer - 1000 > ms_prev )
		{
			...
			{
				statsText = 
					statsTarget.numCreated + ' sprites created ' +
					statsTarget.numReused + ' sprites reused';
			}
...
function CreateGUI(title:String = "", inX:Number=8, inY:Number=8, inCol:int = 0xFFFFFF):GameGUI{
	return new GameGUI(title, inX, inY, inCol)	
}