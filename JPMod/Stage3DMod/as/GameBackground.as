...
class Background extends EntityManager
{
	// how fast the stars move
	public var bgSpeed:Number = 3;
	...
	public function Background(view:Rectangle)
	{
		// run the init functions of the EntityManager class
		super(view);
	}
	
	override public function createBatch(context3D:Context3D,atlasBitmap:Bitmap,spritesA:Number,spritesD:Number) : LiteSpriteBatch 
	{
		
		SpritesPerRow = spritesA;
		SpritesPerCol = spritesD;
		var bgsourceBitmap:Bitmap = atlasBitmap;
		if (atlasBitmap==null) 	throw"unable to loas bitmap ";
		// create a spritesheet with single giant sprite
		spriteSheet = new LiteSpriteSheet(bgsourceBitmap.bitmapData, SpritesPerRow, SpritesPerCol);
	...
	{		
		...
		for(var i:int=0; i<entityPool.length;i++){
			...
			anEntity.sprite.position.x += entityPool[1].speedX;
...
function CreateBackground(view:Rectangle):Background{
